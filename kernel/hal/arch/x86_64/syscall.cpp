...
static bool ReadRtcStable(MorphicDateTime *out) {
  ...
  for (int attempt = 0; attempt < 8; attempt++) {
    ...
    if (sec1 == sec2 && min1 == min2 && hour1 == hour2 && day1 == day2 &&
        mon1 == mon2 && year1 == year2 && cent1 == cent2 && regB1 == regB2)
      break;
  }
...
void Init() {
  ...
  // IMPORTANT:
  // MASK IF during syscall to prevent timer interrupts from causing context
  // switch while kernel stack is in use. This prevents stack corruption when
  // scheduler updates TSS.RSP0 mid-syscall. Syscalls that need interrupts (like
  // SYS_SLEEP) must manually enable them.
...
extern "C" uint64_t syscall_handler(uint64_t num, uint64_t arg1, uint64_t arg2,
                                    uint64_t arg3) {
  // Syscall tracing - DISABLED in production for performance
  // Enable with: -DSYSCALL_TRACE in CXXFLAGS
#ifdef SYSCALL_TRACE
  static uint64_t callCount = 0;
  uint64_t taskId = Scheduler::GetCurrentTaskId();
  if (taskId >= 2 && callCount < 100) {
  ...
  case SYS_GET_SCREEN_INFO:
    ...
    {
      uint64_t v =
          ((uint64_t)Graphics::GetWidth() << 32) | Graphics::GetHeight();
  ...
  case SYS_GET_EVENT: {
    ...
    if (InputManager::GetNextOSEvent(&kEv)) {
      ...
      *uEv = kEv;
      ...
      if (g_sysGetEventDelivered <= 5 ||
          ((g_sysGetEventDelivered & 0xFF) == 0)) {
  ...
  case SYS_GET_TIME_MS:
    // Return system time in milliseconds (assuming PIT 1ms ticks or close
    // enough)
    return PIT_GetTicks();
  ...
  case SYS_ALLOC_BACKBUFFER: {
    ...
    // But we can just use PMM::AllocPage() in a loop since we are remapping
    // them virtuallly contiguous! We DON'T need physical contiguity for
    // software rendering, only virtual.
    ...
    for (uint64_t i = 0; i < pages; i++) {
      ...
      // Map to Virtual
      MMU::MapPage(
          virt_base + (i * 4096), (uint64_t)phys_ptr,
  ...
  case 50: // SYS_VIDEO_MAP
  {
    ...
    // For Desktop (compositor), we need to share the SAME backbuffer as the
    // kernel. This ensures Desktop's drawing and ComposeAppWindowsOnly()
    ...
    for (uint64_t i = 0; i < pages; i++) {
      MMU::MapPage(user_video_virt + i * 4096, k_phys_base + i * 4096,
                   PAGE_USER | PAGE_WRITABLE | PAGE_PRESENT);
    }
  ...
  case 51: // SYS_VIDEO_FLIP
  {
    ...
    // 1. Check if this is the Main Desktop Backbuffer (mapped at
    // 0x600100000000) If so, perform actual screen flip (Hardware VSync Swap)
    const uint64_t DESKTOP_BUFFER_VIRT = 0x600100000000ULL;

    if (buffer == DESKTOP_BUFFER_VIRT) {
      // Only force full screen dirty when explicitly requested (e.g., launcher
      // close)
      if (flags & 1) {
  ...
  case 68: // SYS_UPDATE_WINDOW (id, pos, size)
    Compositor::UpdateWindow(
        arg1, (uint32_t)(arg2 >> 32), (uint32_t)(arg2 & 0xFFFFFFFF),
  ...
  {
    uint64_t ptr = arg1;
    if (ptr < 0x600000000000ULL)
      return 0; // Sanity check user pointer
  ...
  case 62: // SYS_CREATE_WINDOW (width, height, flags)
  {
    ...
    for (uint64_t i = 0; i < pages; i++) {
      MMU::MapPage(user_video_virt + i * 4096, win->phys_addr + i * 4096,
                   PAGE_USER | PAGE_WRITABLE | PAGE_PRESENT);
    }
  ...
  case SYS_SPAWN: {
    ...
    // This is crucial because user_path is in the Caller's address space (e.g.
    // 0x6000...) Once we switch CR3 to the new process, that address space is
    // GONE.
    char path_buf[256];
    int i = 0;
    while (i < 255) {
    ...
    // Use path_buf which is on Kernel Stack (mapped in Identity/Kernel Space,
    // so visible in newCR3)
    ...
    if (proc.error_code == 0) {
      ...
      Scheduler::CreateUserTask((void (*)())proc.entry_point,
  ...
  case SYS_MAP_WINDOW: {
    ...
    // HACK: Use a fixed high range for Compositor mappings: 0x7000_0000_0000 +
    // (ID * 16MB)
    uint64_t compos_base =
        0x700000000000ULL + (winId * 0x1000000); // 16MB per window max slot

    for (uint64_t i = 0; i < pages; i++) {
      MMU::MapPage(compos_base + i * 4096, win->phys_addr + i * 4096,
                   PAGE_USER | PAGE_WRITABLE | PAGE_PRESENT);
    }
  ...
  {
    const char *user_path = (const char *)arg1;
    if (!user_path || arg1 < 0x600000000000ULL)
      return 0;
    if (!arg2 || arg2 < 0x600000000000ULL)
      return 0;
  ...
  {
    if (!arg1 || arg1 < 0x600000000000ULL)
      return 0;
